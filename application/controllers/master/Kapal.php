...
class Kapal extends CI_Controller
{
    ...
    public function editss($id)
    {
        $data = [
            ...
            'jenis_layanan' => $this->Jenis_layanan_model->getAll(),
            'kapal' => $this->Kapal_model->getAll(),
            'edit_kapal' => $this->Kapal_model->getAll($id),
            'siup' => $this->db->get_where('tbl_kapal', ['id_kapal' => $id])->row_array(),
            // 'sikpi' => $this->db->get_where('tbl_kapal', ['id_kapal' => $id])->row_array(),
            // 'sipi' => $this->db->get_where('tbl_kapal', ['id_kapal' => $id])->row_array(),
            // 'andon' => $this->db->get_where('tbl_kapal', ['id_kapal' => $id])->row_array(),
            // 'kelayakan' => $this->db->get_where('tbl_kapal', ['id_kapal' => $id])->row_array(),
            // 'pas' => $this->db->get_where('tbl_kapal', ['id_kapal' => $id])->row_array(),
            // 'ukur' => $this->db->get_where('tbl_kapal', ['id_kapal' => $id])->row_array(),
            // 'akte' => $this->db->get_where('tbl_kapal', ['id_kapal' => $id])->row_array(),
        ];

        $id = $this->input->post('id_kapal');

        $old_siup = $data['siup']['file_siup'];
        $old_sikpi = $data['sikpi']['file_sikpi'];
        $old_sipi = $data['sipi']['file_sipi'];
        $old_sipi_andon = $data['andon']['file_sipi_andon'];
        $old_surat_kelayakan = $data['kelayakan']['file_surat_kelayakan'];
        $old_pas_kecil_pas_besar_surat_laut = $data['pas']['file_pas_kecil_pas_besar_surat_laut'];
        $old_surat_ukur_kapal = $data['ukur']['file_surat_ukur_kapal'];
        $old_gross_akte_kapal = $data['akte']['file_gross_akte_kapal'];
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar');
            // $this->load->view('templates/topbar');
            $this->load->view('admin/dashboard/master/kapal/edit');
            $this->load->view('templates/footer');
        } else {
            $data = [
                'nama_kapal' => $this->input->post('nama_kapal'),
